...
start_idx_helpers = content.find('fn extract_string_property')
end_idx_helpers = content.find('    fn parse_response_into_bank_statement')
end_idx_parse = content.find('Ok(BankStatement {', end_idx_helpers)
end_idx_parse = content.find('}', end_idx_parse) + 1

new_content = content[:start_idx_helpers] + struct_defs + "\nimpl DocumentAiClient {\n" + new_parse_fn + content[end_idx_parse:]
