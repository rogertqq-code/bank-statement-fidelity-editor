...
|---|---|
| Python | Bundled CPython 3.11 for the first polished release |
...
The worker emits a strict startup handshake containing its Python version, PyMuPDF version, PyMuPDF Pro version, exact-version compatibility result, supported operations, platform, and import failure classes. The Rust capability registry may report the core pipeline as ready only after this handshake succeeds. Pro operations are selectable only when all of the following are true:
...
- successful `python/verify_runtime_versions.py` output;
